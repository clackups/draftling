...
#if defined(CONFIG_DRAFTLING_MODEL_SEEED_RETERMINAL_E1001)
/* ---- Seeed reTerminal E1001 buttons ----
 *
 * KEY0 (GPIO3): Multi-function (Wakeup / Confirm / F1 / Back / Sleep)
 *   - Tap (< 400 ms): Confirm (Enter) in menus/browser, or F1 menu in editor
 *   - Medium hold (>= 400 ms, < 2000 ms): Back (Esc)
 *   - Long hold (>= 2000 ms): Deep sleep (standby_enter_sleep())
 * KEY1 (GPIO4): Down (immediate press-down + repeat)
 * KEY2 (GPIO5): Up   (immediate press-down + repeat)
 */
static void reterminal_e1001_inject_key(uint8_t keycode, bool pressed)
{
    kb_event_t ev = {};
    ev.keycode = keycode;
    ev.pressed = pressed;
...
static void reterminal_e1001_btn_poll_cb(void *arg)
{
    (void)arg;
    static const struct { int pin; uint8_t keycode; } kNavButtons[] = {
        { BTN_UP_PIN,   KB_KEY_UP },
        { BTN_DOWN_PIN, KB_KEY_DOWN },
    };
    static bool nav_down[2]   = { false, false };
    static int  nav_stable[2] = { 0, 0 };

    /* Poll Up and Down buttons */
    for (int i = 0; i < 2; i++) {
        bool raw_down = gpio_get_level((gpio_num_t)kNavButtons[i].pin) == 0;
        if (raw_down == nav_down[i]) {
            nav_stable[i] = 0;
            continue;
        }
        if (++nav_stable[i] < 2) continue;
        nav_stable[i] = 0;
        nav_down[i] = raw_down;
        reterminal_e1001_inject_key(kNavButtons[i].keycode, nav_down[i]);
    }

    /* Poll multi-function Wake/Confirm/Back/Sleep button (KEY0 / GPIO3) */
    static bool mf_down             = false;
    static int  mf_stable           = 0;
    static int  mf_hold_ticks       = 0;
    static bool mf_long_press_fired = false;

    bool mf_raw = gpio_get_level((gpio_num_t)WAKEUP_GPIO_NUM) == 0;
    if (mf_raw == mf_down) {
        if (mf_down && !mf_long_press_fired) {
            mf_hold_ticks++;
            /* >= 2000 ms: trigger deep sleep immediately while holding */
            if (mf_hold_ticks >= (2000 / BTN_POLL_PERIOD_MS)) {
                mf_long_press_fired = true;
                ESP_LOGI(TAG, "Multi-function button held >= 2000 ms -- entering sleep");
                standby_enter_sleep();
            }
        }
        mf_stable = 0;
    ...
    if (mf_down) {
        /* Button pressed down */
        mf_hold_ticks = 0;
        mf_long_press_fired = false;
    } else {
        /* Button released */
        if (!mf_long_press_fired) {
            int hold_ms = mf_hold_ticks * BTN_POLL_PERIOD_MS;
            if (hold_ms >= 400) {
                /* > 500 ms tier: Back (Escape) */
                ESP_LOGI(TAG, "Multi-function button hold (%d ms): Back (Esc)", hold_ms);
                reterminal_e1001_inject_key(KB_KEY_ESCAPE, true);
                reterminal_e1001_inject_key(KB_KEY_ESCAPE, false);
            } else {
                /* < 100 ms / short tap tier: Confirm (Enter) or F1 menu */
                if (editor_ui_is_in_editor()) {
                    ESP_LOGI(TAG, "Multi-function button tap (%d ms): F1 menu", hold_ms);
                    reterminal_e1001_inject_key(KB_KEY_F1, true);
                    reterminal_e1001_inject_key(KB_KEY_F1, false);
                } else {
                    ESP_LOGI(TAG, "Multi-function button tap (%d ms): Confirm (Enter)", hold_ms);
                    reterminal_e1001_inject_key(KB_KEY_ENTER, true);
                    reterminal_e1001_inject_key(KB_KEY_ENTER, false);
                }
            }
        }
        mf_hold_ticks = 0;
...
static void reterminal_e1001_btn_init(void)
{
    gpio_config_t g = {};
    g.intr_type    = GPIO_INTR_DISABLE;
    g.mode         = GPIO_MODE_INPUT;
    g.pin_bit_mask = (1ULL << BTN_UP_PIN) | (1ULL << BTN_DOWN_PIN) |
                     (1ULL << WAKEUP_GPIO_NUM);
    g.pull_up_en   = GPIO_PULLUP_ENABLE;
    g.pull_down_en = GPIO_PULLDOWN_DISABLE;
    gpio_config(&g);

    esp_timer_create_args_t targs = {};
    targs.callback = reterminal_e1001_btn_poll_cb;
    targs.name     = "e1001_btn";
    esp_timer_handle_t t = NULL;
    ESP_ERROR_CHECK(esp_timer_create(&targs, &t));
    ESP_ERROR_CHECK(esp_timer_start_periodic(t, (uint64_t)BTN_POLL_PERIOD_MS * 1000));
    ESP_LOGI(TAG, "Seeed reTerminal E1001 button poller started (Up=GPIO%d, Down=GPIO%d, Action=GPIO%d)",
             BTN_UP_PIN, BTN_DOWN_PIN, WAKEUP_GPIO_NUM);
}
#endif /* CONFIG_DRAFTLING_MODEL_SEEED_RETERMINAL_E1001 */
...
static void wakeup_btn_poll_cb(void *arg)
{
    ...
    if (!down) {
        ...
        /* KEY2 (GPIO5) short press opens/closes the F1 Settings menu */
        if (!long_press_fired) {
            reterminal_e1001_inject_key(KB_KEY_F1, true);
            reterminal_e1001_inject_key(KB_KEY_F1, false);
        }
...
static void wakeup_btn_init(void)
{
    ...
    ESP_LOGI(TAG, "Wakeup button poller started (GPIO%d, level now %d; "
        ...
        * !CONFIG_DRAFTLING_MODEL_SEEED_RETERMINAL_E1001 */
 ...
 * the long hold and calls standby_enter_sleep() -- it does not inject
...
#elif defined(CONFIG_DRAFTLING_MODEL_SEEED_RETERMINAL_E1001)
    /* KEY0 (GPIO3) multi-function action (confirm/F1/back/sleep),
     * KEY1 (GPIO4) Down, KEY2 (GPIO5) Up. */
    reterminal_e1001_btn_init();
#else
