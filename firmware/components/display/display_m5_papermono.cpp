#include "sdkconfig.h"
#if defined(CONFIG_DRAFTLING_DISPLAY_M5_PAPERMONO)

/*
 * M5Stack PaperMono / PaperMono-Lite SPI e-paper backend.
 *
 * https://docs.m5stack.com/en/core/PaperMono-Lite -- a 3.97" 800x480
 * black/white e-paper panel on a single SSD1677 controller, the same
 * panel class and differential-refresh protocol as
 * display_ws_epd397.cpp (dual-RAM 0x24/0x26 writes, 0x22/0x20 update
 * control / master activation, busy-while-HIGH, X-increment /
 * Y-decrement RAM addressing with the gate axis reversed). M5GFX's
 * Panel_SSD1677 drives this panel with exactly that RAM addressing,
 ...
 * pulses RST before display_init() (see main/boards/
 * m5stack_papermono.h), so this backend never touches them. The
 * front-light is the M5PM1 power-management chip's PWM0 output,
 * set through battery_m5pm1_set_frontlight() (components/battery/).
 */
...
static const char *TAG = "DisplayM5PaperMono";
...
#define EPD_SCLK_PIN         15
#define EPD_MOSI_PIN         14
#define EPD_CS_PIN           16
#define EPD_DC_PIN           17
#define EPD_BUSY_PIN         18

#define PAPERMONO_SPI_HOST   SPI2_HOST
/* M5Stack's own PaperMono demos run the panel at 20 MHz. */
#define PAPERMONO_SPI_CLOCK_HZ  (20 * 1000 * 1000)
...
#define PAPERMONO_FULL_REFRESH_INTERVAL CONFIG_DRAFTLING_EPD_FULL_REFRESH_INTERVAL
#else
#define PAPERMONO_FULL_REFRESH_INTERVAL 30
#endif

static esp_lcd_panel_io_handle_t s_io = NULL;
static uint8_t  *s_fb = NULL;
/* Bitwise-complement scratch copy of s_fb, used only by
 * papermono_display_full() -- see its comment. */
 ...
 * this board's own pins.
 * ============================================================ */

static void papermono_set_ram_area_full(void)
{
...
static void papermono_ctrl_init(void)
{
    ...
    epd_cmd(0x3C); epd_data1(0x01); /* BORDER_WAVEFORM init value */

    papermono_set_ram_area_full();

    epd_cmd(0x46); epd_data1(0xF7); epd_wait_busy(); /* AUTO_WRITE_BW_RAM */
    epd_cmd(0x47); epd_data1(0xF7); epd_wait_busy(); /* AUTO_WRITE_RED_RAM */
}

static void papermono_refresh(uint8_t ctrl1, uint8_t seq)
{
...
static void papermono_display_full(const uint8_t *fb)
{
    /* Reuse papermono_display_fast()'s 0xFC differential waveform
...
static void papermono_deep_sleep(void)
{
...
{
    /* The panel supply and reset are switched from main.cpp through
     * the M5IOE1 before display_init() (see the file header); nothing
     * to do here. */
    (void)bus_handle;
}
...
extern "C" void display_init(int, int, int, int, int, int, int width, int height)
{
    ...
    ESP_ERROR_CHECK(spi_bus_initialize(PAPERMONO_SPI_HOST, &bus_cfg, SPI_DMA_CH_AUTO));

    esp_lcd_panel_io_spi_config_t io_cfg = {};
    io_cfg.dc_gpio_num       = (gpio_num_t)EPD_DC_PIN;
    io_cfg.cs_gpio_num       = (gpio_num_t)EPD_CS_PIN;
    io_cfg.pclk_hz           = PAPERMONO_SPI_CLOCK_HZ;
    io_cfg.lcd_cmd_bits      = 8;
    io_cfg.lcd_param_bits    = 8;
    io_cfg.spi_mode          = 0;
    io_cfg.trans_queue_depth = 4;
    io_cfg.flags.psram_dma_direct = 1;
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)PAPERMONO_SPI_HOST, &io_cfg, &s_io));
    ...
    ESP_LOGI(TAG, "M5Stack PaperMono e-paper initialized (%dx%d)",
             s_width, s_height);
}
...
extern "C" void display_flush(void)
{
    ...
    bool do_full = s_needs_initial_full || s_force_full || huge ||
                   s_partial_count >= PAPERMONO_FULL_REFRESH_INTERVAL;

    if (do_full) {
        papermono_display_full(s_fb);
        s_partial_count = 0;
        s_needs_initial_full = false;
    } else {
        papermono_display_fast(s_fb);
        s_partial_count++;
    }
...
{
    if (battery_m5pm1_set_frontlight(percent) != 0) {
        ESP_LOGW(TAG, "Front-light update to %d%% failed", percent);
    }
}

extern "C" void display_deep_sleep_prepare(void)
{
    if (s_initialized) papermono_deep_sleep();
    s_initialized = false;
}

#endif /* CONFIG_DRAFTLING_DISPLAY_M5_PAPERMONO */
